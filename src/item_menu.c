...
static void Task_PrintSaleConfirmationText(u8 taskId)
{
    ...
    ConvertIntToDecimalStringN(gStringVar3, itemid_get_market_price(BagGetItemIdByPocketPosition(gBagMenuState.pocket + 1, data[1])) / 10 * data[8], STR_CONV_MODE_LEFT_ALIGN, 6);
...
static void Task_InitSaleQuantitySelectInterface(u8 taskId)
{
    ...
    // TWEAK: reduce sale prices by 10 times rather than 2 times
    UpdateSalePriceDisplay(itemid_get_market_price(BagGetItemIdByPocketPosition(gBagMenuState.pocket + 1, data[1])) / 10 * data[8]);
    BagPrintMoneyAmount();
